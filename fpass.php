...
if(isset($_POST['submit'])) {
	$email = $_POST['email'];
	$query = "SELECT * FROM reg WHERE mail = '".$email."'";
	$result = $conn->query($query);
if($result->num_rows > 0){
	while($row = $result->fetch_assoc()){
		$uid = $row['user_id'];
		$uid_encode = base64_encode($uid);
		$link = "<a href='Reset_pass.php?MnoQtyPXZORTE=$uid_encode' class='btn btn-info btn-sm'>Recieve Mail</a>";
	}
	}else{
