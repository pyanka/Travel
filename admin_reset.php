...
session_start();
$aid = $_GET['MnoQtyPXZORTE'];
$message = $Home = '';
$_SESSION['admin'] = $aid;
if ($_SESSION['admin'] == '') {
		header("location:admin_fpass.php");
}
else
{
if(isset($_POST['submit'])) {
	...
	$id_decode = base64_decode($aid);
    $query = "UPDATE admin_reg SET password = '$password' WHERE admin_id = '$id_decode' ";
	$result = $conn->query($query);
		if($result){
			$message = "<div class='alert alert-success'>Reset Your Password Successfully..</div>";
			$Home = "<a href='adminlog.html' class='btn btn-success btn-sm'>Login</a>";
	}else{
