...
if(isset($_POST['submit'])) {
	...
	$id_decode = base64_decode($uid);
    $query = "UPDATE reg SET password = '$password' WHERE user_id = '$id_decode' ";
	$result = $conn->query($query);
		if($result){
			$message = "<div class='alert alert-success'>Reset Your Password Successfully..</div>";
			$Home = "<a href='userlog.html' class='btn btn-success btn-sm'>Login</a>";
	}else{
